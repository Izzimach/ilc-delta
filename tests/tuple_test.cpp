...
TEST_CASE("Tuple basic operations", "[basic][tuple]") {
    auto t1 = ilc::Tuple<ilc::r_int,ilc::r_double>::init({10,20.5});
    auto t2 = t1.patch({ilc::rp_int{15}, ilc::rp_double{25.5}});
    ...
    std::tuple<ilc::rp_int,ilc::rp_double> diff =
        ilc::Tuple<ilc::r_int,ilc::r_double>::diff({10,20.5},{15,25.5});
    REQUIRE(std::get<0>(diff).has_value());
    REQUIRE(*std::get<0>(diff) == 15);
    REQUIRE(std::get<1>(diff).has_value());
    REQUIRE(*std::get<1>(diff) == 25.5);
 
    auto t3 = t2.patch({ilc::rp_int{20}, ilc::rp_double{30.5}});
